...
SELECT yt_user.user_name, ch_name 
FROM yt_user, channel 
WHERE ch_category = 'Personal' 
AND yt_user.user_id = channel.user_id; 

/*Personal channels belonging to Alice*/
SELECT yt_user.user_id, user_name, ch_name
FROM yt_user, channel
WHERE user_name = 'Alice'
AND ch_category = 'Personal' 
AND yt_user.user_id = channel.user_id; 

/*List of channels and number of their subscriptions 
(does not include channels with zero subscription)*/
SELECT channel.ch_name, COUNT(subscription_id) AS NUMBER_OF_SUBSCRIPTION
FROM channel 
LEFT JOIN subscription
ON subscription.ch_id = channel.ch_id
GROUP BY channel.ch_id, channel.ch_name
HAVING COUNT(subscription_id) <> 0
ORDER BY channel.ch_name;

/*List of channels and number of their subscriptions (include channels with 
zero subscription)*/
SELECT channel.ch_name, COUNT(subscription_id) AS NUMBER_OF_SUBSCRIPTION
FROM channel 
LEFT JOIN subscription
...
SELECT channel.ch_name, yt_user.user_name, COUNT(subscription_id) AS NUMBER_OF_SUBSCRIPTION
FROM channel
LEFT JOIN subscription
ON subscription.ch_id = channel.ch_id
LEFT JOIN yt_user
ON yt_user.user_id = channel.user_id
GROUP BY channel.ch_id, channel.ch_name, yt_user.user_name
ORDER BY ch_name;

/*The total number of subscriptions*/
SELECT COUNT(subscription_id) as Total_number_of_subscriptions FROM subscription;
